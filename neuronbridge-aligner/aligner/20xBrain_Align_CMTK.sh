...
function reformatAll() {
    ...
    for ((i=1; i<=${NCHANNELS}; i++)); do
        ...
        reformat "$GLOBAL_NRRD" "$_TEMP" "$_DEFFIELD" "${OUTPUT}_NRRD" "$i" "ignore" "$opts"
    done
}
...
# "-------------------Template----------------------"
JRC2018_Unisex_Onemicron1=$TempDir"/JRC2018_UNISEX_20x_onemicron.nrrd"
JRC2018_Unisex_OnemicronNoOL=$TempDir"/JRC2018_UNISEX_20x_gen1_noOPonemicron.nrrd"
...
if [[ -e ${registered_affine_xform} ]]; then
    ...
    ${CMTK}/reformatx -o "${sig}" --floating ${gsig} ${TEMPLATE} ${DEFFIELD}
    ${FIJI} -macro ${REGCROP} "${TEMPLATE},${sig},${NSLOTS}"
fi
...
TEMPLATE="$TempDir/JRC2018_UNISEX_20x_HR.nrrd"
