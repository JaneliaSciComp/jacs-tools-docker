...
if [[ "$INPUT_FILE_EXT" = "$OUTPUT_FILE_EXT" && "$SPLIT_CHANNELS" = "0" ]]; then
    ...
    if [[ "$INPUT_FILE_EXT" = "$OUTPUT_FILE_EXT" && "$SPLIT_CHANNELS" = "0" ]]; then
        ...
        if [[ "$OUTPUT_FILE_EXT" == "v3dpbd" || "$OUTPUT_FILE_EXT" == "mp4" || "$OUTPUT_FILE_EXT" = "h5j" ]]; then
            ...
            $Fiji $JAVA_OPTS --headless -macro $FijiMacros/convert_stack.ijm "$INPUT_FILE,$TEMP_FILE,$SPLIT_CHANNELS"
            ...
            $Fiji $JAVA_OPTS --headless -macro $FijiMacros/convert_stack.ijm "$INPUT_FILE,$OUTPUT_FILE,$SPLIT_CHANNELS"
        fi
