...
ANTS="/app/ants/ANTS"
WARP="/app/ants/WarpImageMultiTransform"
SAMPLE="/app/ants/ResampleImageBySpacing"
...
while getopts "o:m:r:s:h" opt
do case "$opt" in
    o) FINAL_OUTPUT="$OPTARG";;
    m) MULTISCAN_BLEND_VERSION="$OPTARG";;
    r) REF_CHAN1="$OPTARG";;
    s) REF_CHAN2="$OPTARG";;
    h) echo "Usage: $0 [-o output_file] [-m multiscanblend_version] [-r input1Ref] [-s input2Ref] input1.lsm input2.lsm" >&2
        exit 1;;
    esac
done
shift $((OPTIND-1))
...
echo "Input 1: $INPUT1"
echo "Input 2: $INPUT2"
echo "Ref Chan 1: $REF_CHAN1"
echo "Ref CHan 2: $REF_CHAN2"
echo "Multiscan Blend Version: multiscanblend$MULTISCAN_BLEND_VERSION"

REF_PARAM=""
if [ ! -z "$REF_CHAN1" ] && [ ! -z "$REF_CHAN2" ]; then
    REF_PARAM="#r $REF_CHAN1 $REF_CHAN2"
fi

echo "REF_PARAM=$REF_PARAM"
...
then
    ...
    $Vaa3D -x multiscanstacks.so -f transformdet -i "$INPUT1" "$INPUT2" -o "$FILETRNSTYPE" -p "$REF_PARAM"
    ...
    do
        ...
        then
            ...
            then
                ...
                $Vaa3D -x multiscanstacks.so -f multiscanblend$MULTISCAN_BLEND_VERSION -i "$INPUT1" "$INPUT2" -o "$OUTPUT" -p "$REF_PARAM"
            ...
            then
                ...
                $Vaa3D -x multiscanstacks.so -f multiscanblend$MULTISCAN_BLEND_VERSION -i "$INPUT1" "$INPUT2" $DEFORMEDV3DRAW -o "$OUTPUT" -p "#d 1 $REF_PARAM"
